...
Proof.
  ...
  - intros H3. apply H1. apply H2. apply H3. 
Qed.
...
Proof.
  unfold not.
  intros X P H [x H2]. apply H2 in H. destruct H.
Qed.
...
Proof.
  ...
  - simpl. split.
    + intro H. rewrite <- IHl'. split.
      ...
      * intros x H1. apply H. right. apply H1.
    + intros H x H1. destruct H1.
      * rewrite <- H0. apply H.
...
Theorem combine_odd_even_elim_odd :
  forall (Podd Peven : nat -> Prop) (n : nat),
    ...
    Podd n.
Proof.
  unfold combine_odd_even. intros.
  destruct (oddb n).
  + apply H.
  + discriminate H0.   
Qed.
...
Theorem tr_rev_correct_helper: forall (X: Type) (t: list X) (x: list X),
  rev_append t x = rev_append t [ ] ++ x.
Proof.
  intros X t.
  induction t as [|h' t' IHt'].
  - reflexivity.
  - simpl. rewrite -> IHt'. symmetry. rewrite <- app_assoc. simpl. symmetry. apply IHt'. 
Qed.
...
Proof.
  ...
  - reflexivity.
  - simpl. unfold tr_rev. simpl. rewrite <- IHt'. unfold tr_rev. apply tr_rev_correct_helper.
Qed.
(** [] *)
...
Proof.
  ...
  - rewrite -> evenb_S. destruct (evenb n') eqn: E.
    + simpl. destruct IHn' as [k H]. rewrite H. exists k. reflexivity.
    + simpl. destruct IHn' as [k H]. exists (S k). rewrite H. reflexivity.
Qed.
...
Proof.
  split.
  * destruct b1 eqn:H.
  - simpl. intros H2. split.
    + reflexivity.
    + apply H2.
  - simpl. discriminate.
  * destruct b1 eqn: H.
    - simpl. intros H2. destruct H2. apply H1.
...
Proof.
  split.
  - destruct b1 eqn:H.
    + simpl. intros H2. left. reflexivity.
    + simpl. intros H2. right. apply H2.
  - destruct b1 eqn:H.
    + simpl. intros [H2 | H2]. reflexivity. reflexivity.
    + simpl. intros [H2 | H2]. discriminate H2. apply H2.
Qed.
(** [] *)
...
Proof.
  intros.
  rewrite <- not_true_iff_false. rewrite <- eqb_eq. reflexivity.
Qed.
...
Theorem eqb_list_true_iff_helper:
...
Proof.
  ...
  - destruct l2 as [|h2 t2].
    + simpl. split.
      * intros H2. discriminate H2.
      * intros H2. discriminate H2.
    + simpl. destruct (eqb h1 h2) eqn: E.
      * rewrite -> IHt1'. apply H in E. rewrite E. split.
        { intros tmp. rewrite tmp. reflexivity.
        } intros tmp. apply eqb_list_true_iff_helper in tmp. apply tmp.
      * split.
        { intros. discriminate H0.
        } intros. injection H0 as H1 H2. apply H in H1. rewrite -> H1 in E. discriminate E.  
Qed. 
...
Proof.
  ...
  - simpl. rewrite <- IHt'. split.
    + intros H. rewrite andb_true_iff in H. apply H.
    + intros H. rewrite andb_true_iff. apply H.
Qed.
...
Proof.
  ...
  - intros. assert ( P \/ ~P). { apply H. } destruct H1. apply H1. unfold not in H0. unfold not in H1.
    apply H0 in H1. destruct H1.
  - intros. unfold not in H. unfold not. left.   
Qed.
*)
(* 2020-08-24 15:39 *)
