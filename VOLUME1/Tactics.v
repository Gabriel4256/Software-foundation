...
Proof.
  ...
  - intros H. rewrite -> H. symmetry. apply rev_involutive.
  - intros H. rewrite H. symmetry. apply rev_involutive.
Qed.
(** [] *)
...
Proof.
  ...
  injection H as H1 H2.
  rewrite <- H2.
  intros H'. injection H'. intros K.
  rewrite <- H1 in K. symmetry in K. apply K. 
Qed.
...
Proof.
  ...
  - intros m. destruct m as [| m'].
    + discriminate.
    + intros H. simpl in H. injection H as H1. rewrite<- plus_n_Sm in H1.
      symmetry in H1. rewrite <- plus_n_Sm in H1. injection H1 as H2. symmetry in H2.
      apply IHn' in H2. f_equal. apply H2.
Qed.
(** [] *)
...
Proof.
  ...
  - intros n. destruct n as [| n'].
    + simpl. discriminate.
    + simpl. intros H. injection H. apply IHl'.
Qed.
(** [] *)
...
Theorem combine_split : forall X Y (l : list (X * Y)) l1 l2,
  ...
  combine l1 l2 = l.
Proof.
  intros X Y l l1 l2 H. generalize dependent l2. generalize dependent l1.
  induction l as [| h l' IHl'].
  - intros. simpl in H. injection H as H1 H2. rewrite <- H1. rewrite <- H2. reflexivity.
  - intros. destruct h as [a b]. simpl in H. destruct (split l') as [c d]. injection H as H1 H2.
    rewrite <- H1. rewrite <- H2. simpl. rewrite -> IHl'. reflexivity. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - destruct n.
    ...
    + destruct p. 
      { discriminate.
      } simpl in H2. simpl. apply IHm'. apply H2. 
Qed.
(** [] *)
...
Proof.
  intros X Y l1.
  induction l1 as [| h1 t1 IHt'].
  - simpl. destruct l2.
    + reflexivity.
    + discriminate.
  - intros. destruct l2 as [| h2 t2].
    + simpl. discriminate.
    + simpl. rewrite -> IHt'. reflexivity. simpl in H. injection H as H1. apply H1.
Qed.
...
Proof.
  ...
  induction l as [| h l' IHl'].
  - simpl. discriminate.
  - simpl. intros. destruct (test h) eqn: H2.
    + injection H as H1. rewrite -> H1 in H2. apply H2.
...
Proof.
  ...
  - simpl. destruct (test h) eqn: H.
    + unfold existsb'. simpl. rewrite -> H. reflexivity.
    + unfold existsb'. simpl. rewrite -> H. simpl. rewrite -> IHl'. unfold existsb'. reflexivity.
Qed.
(* 2020-08-24 15:39 *)
